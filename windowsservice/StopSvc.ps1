...
#   Description: Script to stop window service
##########################################################################################################
...
[CmdletBinding()]
	...
	Try{
		...
		"SERVICE STOP COMMAND INITIATED ($Date): "
		if($Service -eq "ssh")
		{
			invoke-command -session $Remote -scriptblock{$services =  Get-service}
			invoke-command -session $Remote -Scriptblock{$services | where-object{$_.Name -eq 'SSHTectiaServer'} | stop-service -pass} | format-table -property PSComputerName, Name, Status  | out-string

		}
		elseif ($Service -eq "iis")
		{
			invoke-command -session $Remote -scriptblock{$services =  Get-service}
			invoke-command -session $Remote -Scriptblock{$services | where-object{$_.Name -eq 'W3SVC'} | stop-service -pass} | format-table -property PSComputerName, Name, Status  | out-string
		}
		else
		{
